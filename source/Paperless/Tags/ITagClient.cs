...
public interface ITagClient
{
  ...
  IAsyncEnumerable<Tag> GetAllAsync(CancellationToken cancellationToken = default);

  /// <summary>Gets all tags.</summary>
  /// <param name="pageSize">The number of tags to get in a single request.</param>
  /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
  /// <returns>A enumerable which will asynchronously iterate over all available pages of tags.</returns>
  IAsyncEnumerable<Tag> GetAllAsync(int pageSize, CancellationToken cancellationToken = default);
  ...
  Task<Tag> CreateAsync(TagCreation tag);
